...
class RightBottomFrame(ctk.CTkFrame,Content,PropertyMixin):
    ...
    def create_regression(self):
        ...
        if not self.content_name_check(self.entry.get()):
            ...
            try:
                data = pandas.read_csv(self.filename, names=['x', 'y'], header=0)
                content_object = {
                    'title': self.entry.get(),
                    'x': list(map(float, data['x'].tolist())),
                    'y': list(map(float, data['y'].tolist()))
                }
            except Exception:
...
class LeftFrame(ctk.CTkScrollableFrame, Content, PropertyMixin):
    ...
    #in LeftFrame Buttons erstellen, die Grafiken zeigen.
